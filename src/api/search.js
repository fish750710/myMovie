...
export default {
  ...
  async searchData(query, page=1) {
    return await fetch(
      ...
      `${baseURL}/multi?${baseParams}&query=${query}&page=${page}&include_adult=false`,
      headers,
    )
      .then((res) => res.json())
  ...
  async getMovies(query, page=1) {
    ...
    return await fetch(`${baseURL}/movie?${baseParams}&query=${query}&page=${page}&include_adult=false`, headers)
      .then((res) => res.json())
      .catch((err) => console.log(err));
  },
  // 搜尋演員
  async getPerson(query, page=1) {
    ...
    return await fetch(
      `${baseURL}/person?${baseParams}&query=${query}&page=${page}&include_adult=false`,
      headers,
    )
      .then((res) => res.json())
      .catch((err) => console.log(err));
  ...
  async getTV(query, page=1) {
    return await fetch(
      ...
      `${baseURL}/tv?${baseParams}&query=${query}&page=${page}&include_adult=false`,
      headers,
    )
      .then((res) => res.json())
      .catch((err) => console.log(err));
